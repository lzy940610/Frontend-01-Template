...
it("Tag start end doesn't match!", e => {
  let doc = parseHTML("<div></diva>")
});

it("Tag start end doesn't match!", e => {
  let doc = parseHTML("<List></List>")
  let div = doc.children[0]
  assert.equal(div.tagName, "List")
...
it('attribute with no value', () => {
  let doc = parseHTML("<div class=\"name\" ></div>")
});
